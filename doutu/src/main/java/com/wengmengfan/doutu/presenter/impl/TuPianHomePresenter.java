...
public class TuPianHomePresenter extends RxPresenter<TuPianHomeContract.View> implements TuPianHomeContract.Presenter<TuPianHomeContract.View> {

    private Api bookApi;
    public static boolean isLastSyncUpdateed = false;

    @Inject
    public TuPianHomePresenter(Api bookApi) {
        this.bookApi = bookApi;
    }


    @Override
    public void Jousp_Home() {
        Observable.create(new Observable.OnSubscribe< List<TuPianHomeBean>>() {
            ...
            public void call(Subscriber<? super  List<TuPianHomeBean>>subscriber) {
                ...
                try {
                    Document doc = Jsoup.connect("http://www.ligui.org/index.html").get();
                    Elements menu = doc.select("ul.menu");
